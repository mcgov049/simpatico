...
#include "NvtLangevinIntegratorAnneal.h"
...
namespace DdMd
{
   ...
   * Constructor.
   */
   NvtLangevinIntegratorAnneal::NvtLangevinIntegratorAnneal(Simulation& simulation)
    : TwoStepIntegrator(simulation),
     ...
     cv_(),
     cr_()
   {  setClassName("NvtLangevinIntegratorAnneal"); }

   /*
   * Destructor.
   */
   NvtLangevinIntegratorAnneal::~NvtLangevinIntegratorAnneal()
   {}

   /*
   * Read time step dt.
   */
   void NvtLangevinIntegratorAnneal::readParameters(std::istream& in)
   {
   ...
   */
   void NvtLangevinIntegratorAnneal::loadParameters(Serializable::IArchive &ar)
   {
   ...
   */
   void NvtLangevinIntegratorAnneal::save(Serializable::OArchive &ar)
   {
      ar << dt_;
      ar << gamma_;
      Integrator::save(ar);
   ...
   */ 
   void NvtLangevinIntegratorAnneal::setup()
   {
      // Precondition
      const EnergyEnsemble& energyEnsemble = simulation().energyEnsemble();
      if (!energyEnsemble.isIsothermal() && !energyEnsemble.isAnneal()) {
         UTIL_THROW("Energy ensemble is not isothermal or anneal");
      }
      ...
      double temp = energyEnsemble.temperature() + energyEnsemble.dT()*iStep_;
   ...
   * Update Temperarture
   */
   void NvtLangevinIntegratorAnneal::updateT()
   {
      const EnergyEnsemble& energyEnsemble = simulation().energyEnsemble();
      int nAtomType = prefactors_.capacity();
      double temp = energyEnsemble.temperature() + energyEnsemble.dT()*iStep_;
   ...
   */
   void NvtLangevinIntegratorAnneal::integrateStep1()
   {
      Vector dv;
      Vector dr;
   ...
   */
   void NvtLangevinIntegratorAnneal::integrateStep2()
   {
